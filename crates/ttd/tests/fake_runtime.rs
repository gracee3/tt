...
use tt_runtime::protocol::{
    jsonrpc::{JsonRpcMessage, JsonRpcNotification, JsonRpcRequest, JsonRpcResponse, RequestId},
    methods, types,
};

pub struct FakeTTAppServer {
    pub endpoint: String,
    task: JoinHandle<()>,
}

impl FakeTTAppServer {
    pub async fn spawn() -> Self {
        let listener = TcpListener::bind(("127.0.0.1", 0))
            .await
            .expect("bind fake TT app server");
        let endpoint = format!(
            "ws://127.0.0.1:{}",
            listener
                .local_addr()
                .expect("fake TT listener address")
                .port()
        );
        let task = tokio::spawn(async move {
            loop {
                let (stream, _) = listener.accept().await.expect("accept fake TT client");
    ...
    async fn serve(mut socket: WebSocketStream<TcpStream>) {
        ...
        while let Some(frame) = socket.next().await {
            ...
            let message: JsonRpcMessage =
                serde_json::from_str(&raw).expect("decode fake TT JSON-RPC");
            match message {
    ...
    async fn handle_request(
        ...
        match request.method.as_str() {
                ...
                Self::send_response(
                    ...
                    &types::InitializeResponse {
                        server_info: Some(types::ServerInfo {
                            name: Some("fake-tt".to_string()),
                            version: Some("0.1.0".to_string()),
                        }),
                        user_agent: Some("fake-tt-tests".to_string()),
            ...
            methods::THREAD_READ => {
                ...
                let current = thread
                    .clone()
                    .expect("fake TT thread should exist before thread/read");
            ...
            methods::TURN_START => {
                ...
                let updated_thread = {
                    let thread = thread
                        .as_mut()
                        .expect("fake TT thread should exist before turn/start");
            ...
            other => panic!("unexpected fake TT request method {other}"),
        }
    }
    ...
    async fn send_response(
        ...
        let message = JsonRpcMessage::Response(JsonRpcResponse {
            ...
            id,
            result: serde_json::to_value(result).expect("serialize fake TT response"),
        });
    ...
    async fn send_notification(
        ...
        let message = JsonRpcMessage::Notification(JsonRpcNotification::new(
            method,
            Some(serde_json::to_value(params).expect("serialize fake TT notification")),
        ));
        Self::send_message(socket, message).await;
    }
        ...
        socket
            .send(Message::Text(
                serde_json::to_string(&message)
                    .expect("serialize fake TT message")
                    .into(),
            ))
            .await
            .expect("send fake TT websocket message");
    }
}

impl Drop for FakeTTAppServer {
    fn drop(&mut self) {
        self.task.abort();
    }
}
