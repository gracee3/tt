...
impl TestDaemon {
    ...
    pub async fn start(&mut self) {
        ...
        let mut command = Command::new(Self::ttd_binary_path());
        command
            .args(&self.extra_args)
            .env("TT_HOME", self.paths.config_dir.clone())
            .env("TT_CONNECTION_MODE", "connect_only")
            .stdin(Stdio::null())
            .stdout(Stdio::null())
        ...
        let child = command.spawn().expect("spawn ttd");
        self.child = Some(child);
        self.wait_until_ready().await;
    }

    fn ttd_binary_path() -> PathBuf {
        if let Some(path) = option_env!("CARGO_BIN_EXE_ttd") {
    ...
    pub async fn connect(&self) -> std::sync::Arc<TTIpcClient> {
        TTIpcClient::connect(&self.paths)
            .await
            .expect("connect real TTIpcClient")
    }

    pub fn tt_home_env(&self) -> Vec<(String, String)> {
        vec![(
            "TT_HOME".to_string(),
    ...
    pub async fn wait_until_ready(&self) {
        ...
        while Instant::now() < deadline {
            if self.paths.socket_file.exists()
                && let Ok(client) = TTIpcClient::connect(&self.paths).await
    ...
    pub async fn next_event_matching<F>(
        events: &mut ttd::EventSubscription,
        predicate: F,
    ) -> tt_core::ipc::DaemonEventEnvelope
    where
        F: Fn(&tt_core::ipc::DaemonEventEnvelope) -> bool,
    {
