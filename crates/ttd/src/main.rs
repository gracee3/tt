...
use ttd::{TTDaemonService, TTRuntimeOverrides};

#[derive(Debug, Parser)]
#[command(name = "ttd", version, about = "TT daemon process")]
struct DaemonCli {
    #[command(flatten)]
...
struct DaemonRuntimeArgs {
    #[arg(
        long,
        help = "Override the local TT binary path for this daemon process"
    )]
    tt_bin: Option<std::path::PathBuf>,
    #[arg(long, help = "Override the upstream TT app-server WebSocket URL")]
...
async fn async_main() -> Result<()> {
    ...
    paths.ensure().await?;
    init_file_logger("ttd", &paths.daemon_log_file)?;
    info!("starting tt daemon process");

    let service = TTDaemonService::load_with_runtime_overrides(
        TTRuntimeOverrides::from_env().overlay(&TTRuntimeOverrides {
            tt_bin: cli.runtime.tt_bin,
...
mod tests {
    ...
    fn parses_direct_daemon_runtime_flags() {
        ...
        assert_eq!(
            cli.runtime.tt_bin.as_deref(),
            Some(std::path::Path::new("/tmp/tt"))
        );
        assert_eq!(
    ...
    fn daemon_help_mentions_the_daemon_process() {
        ...
        assert!(help.contains("TT daemon process"));
    ...
    fn daemon_runtime_mode_flags_conflict() {
        let result = DaemonCli::try_parse_from(["ttd", "--connect-only", "--force-spawn"]);
