...
//! restarting the TT daemon under the current user-scoped runtime model. It
...
//!
//! Read this alongside `ttd::client` for socket-lifetime request behavior
//! and `tt_core::ipc` for the daemon/runtime metadata carried over the wire.
...
use tt_core::{AppConfig, AppPaths, TTConnectionMode, TTError, TTResult, ipc};

use crate::client::TTIpcClient;

pub const ENV_RUNTIME_BIN: &str = "TT_RUNTIME_BIN";
pub const ENV_RUNTIME_LISTEN_URL: &str = "TT_RUNTIME_LISTEN_URL";
pub const ENV_DEFAULT_CWD: &str = "TT_DEFAULT_CWD";
pub const ENV_DEFAULT_WORKTREE_ROOT: &str = "TT_DEFAULT_WORKTREE_ROOT";
pub const ENV_DEFAULT_MODEL: &str = "TT_DEFAULT_MODEL";
pub const ENV_CONNECTION_MODE: &str = "TT_CONNECTION_MODE";
pub const ENV_DAEMON_BINARY_PATH: &str = "TT_DAEMON_BINARY_PATH";
pub const ENV_DAEMON_BUILD_FINGERPRINT: &str = "TT_DAEMON_BUILD_FINGERPRINT";
pub const ENV_INBOX_MIRROR_SERVER_URL: &str = "TT_INBOX_MIRROR_SERVER_URL";

#[derive(Debug, Clone, Default)]
pub struct TTRuntimeOverrides {
...
impl TTRuntimeOverrides {
    ...
    pub fn from_env() -> Self {
        let tt_bin = std::env::var_os(ENV_RUNTIME_BIN).map(PathBuf::from);
        let listen_url = std::env::var(ENV_RUNTIME_LISTEN_URL).ok();
    ...
    pub fn overlay(mut self, overrides: &Self) -> Self {
        if let Some(tt_bin) = &overrides.tt_bin {
            self.tt_bin = Some(tt_bin.clone());
        }
...
#[derive(Debug, Clone)]
pub struct TTDaemonSocketStatus {
...
pub fn apply_runtime_overrides(config: &mut AppConfig, overrides: &TTRuntimeOverrides) {
    if let Some(tt_bin) = &overrides.tt_bin {
        config.tt.binary_path = tt_bin.clone();
    }
    if let Some(listen_url) = &overrides.listen_url {
        config.tt.listen_url = listen_url.clone();
        config.tt.app_server.default.listen_url = listen_url.clone();
    }
    ...
    if overrides.connect_only {
        config.tt.connection_mode = TTConnectionMode::ConnectOnly;
    }
}

#[derive(Debug, Clone)]
pub struct TTDaemonProcessManager {
    paths: AppPaths,
    overrides: TTRuntimeOverrides,
}

impl TTDaemonProcessManager {
    pub fn new(paths: AppPaths, overrides: TTRuntimeOverrides) -> Self {
        Self { paths, overrides }
    }

    pub async fn status(&self) -> TTResult<TTDaemonSocketStatus> {
    ...
    pub async fn ensure_running(&self, launch: TTDaemonLaunch) -> TTResult<TTDaemonSocketStatus> {
        ...
        match launch {
            ...
            TTDaemonLaunch::Always => self.restart().await,
        }
    }

    pub async fn restart(&self) -> TTResult<TTDaemonSocketStatus> {
    ...
    pub async fn stop(&self) -> TTResult<TTDaemonSocketStatus> {
    ...
    pub async fn spawn_background(&self) -> TTResult<TTDaemonSocketStatus> {
        ...
        let mut child = command
            .spawn()
            .map_err(|error| TTError::Transport(format!("failed to spawn TT daemon: {error}")))?;
        ...
        while Instant::now() < deadline {
            ...
            if let Some(exit_status) = child.try_wait()? {
                return Err(TTError::Transport(format!(
                    "TT daemon exited early with status {exit_status}"
                )));
            }
        ...
        Err(TTError::Transport(format!(
            "timed out waiting for TT daemon socket {}",
            self.paths.socket_file.display()
        )))
    }

    async fn stop_process(&self, status: &TTDaemonSocketStatus) -> TTResult<()> {
    ...
    async fn cleanup_stale_runtime(&self) -> TTResult<()> {
    ...
    async fn request_graceful_stop(&self) -> TTResult<()> {
        debug!("requesting graceful stop via daemon rpc");
        let client = TTIpcClient::connect(&self.paths).await?;
        let _ = client.daemon_stop().await?;
        Ok(())
    }

    async fn wait_for_stop(&self) -> TTResult<TTDaemonSocketStatus> {
        let start = std::time::Instant::now();
        ...
        Err(TTError::Transport(format!(
            "timed out waiting for TT daemon at {} to stop",
            self.paths.socket_file.display()
        )))
    }

    async fn resolve_daemon_binary(&self, build_if_needed: bool) -> TTResult<PathBuf> {
        if let Some(ttd) = std::env::current_exe()
            .ok()
            .as_ref()
            .and_then(|exe| exe.parent().map(|parent| parent.join("ttd")))
        ...
        let repo_root = Self::repo_root();
        let built_binary = repo_root.join("target/debug/ttd");
        ...
        let status = Command::new("cargo")
            ...
            .await
            .map_err(|error| {
                TTError::Transport(format!("failed to build or locate ttd: {error}"))
            })?;
        if !status.success() {
            return Err(TTError::Transport(format!(
                "failed to prepare ttd binary, cargo exited with {status}"
            )));
        }
    ...
    fn apply_spawn_env(&self, command: &mut Command, binary: &ipc::DaemonBinarySummary) {
        ...
        if let Some(tt_bin) = &self.overrides.tt_bin {
            command.env(ENV_RUNTIME_BIN, tt_bin);
        }
        if let Some(listen_url) = &self.overrides.listen_url {
            command.env(ENV_RUNTIME_LISTEN_URL, listen_url);
        }
    ...
    pub async fn runtime_metadata_for_current_process(
        paths: &AppPaths,
    ) -> TTResult<ipc::DaemonRuntimeMetadata> {
        ...
        let binary_path = std::env::var(ENV_DAEMON_BINARY_PATH)
            ...
            .map_err(|error| {
                TTError::Transport(format!("failed to resolve current daemon binary: {error}"))
            })?;
        ...
        Ok(ipc::DaemonRuntimeMetadata {
            ...
            git_commit: option_env!("TT_GIT_COMMIT").map(ToOwned::to_owned),
        })
    }
    ...
    pub async fn read_runtime_metadata(paths: &AppPaths) -> TTResult<ipc::DaemonRuntimeMetadata> {
    ...
    async fn signal_pid(pid: u32, signal: &str) -> TTResult<()> {
        let status = Command::new("kill")
            ...
            .await
            .map_err(|error| {
                TTError::Transport(format!("failed to signal daemon pid {pid}: {error}"))
            })?;
        if status.success() {
            Ok(())
        } else {
    ...
    async fn fetch_daemon_status(paths: &AppPaths) -> Option<ipc::DaemonStatusResponse> {
        ...
        let client = TTIpcClient::connect(paths).await.ok()?;
        client.daemon_status().await.ok()
    }

    async fn socket_owner_pid(path: &Path) -> TTResult<Option<u32>> {
        let output = Command::new("lsof")
            ...
            .map_err(|error| {
                TTError::Transport(format!(
                    "failed to inspect TT daemon socket owner for {}: {error}",
                    path.display()
                ))
            })?;
    ...
    fn binary_summary_from_path(path: &Path) -> TTResult<ipc::DaemonBinarySummary> {
...
#[cfg(test)]
mod tests {
    use super::{TTDaemonProcessManager, TTRuntimeOverrides};
    use chrono::Utc;
    use tt_core::AppPaths;

    fn temp_paths(test_name: &str) -> AppPaths {
        let root = std::env::temp_dir().join(format!(
            "tt-process-test-{test_name}-{}-{}",
            std::process::id(),
        ...
        AppPaths {
            ...
            socket_file: root.join("runtime/ttd.sock"),
            daemon_metadata_file: root.join("runtime/ttd.json"),
            daemon_log_file: root.join("data/logs/ttd.log"),
        }
    }
    ...
    async fn runtime_metadata_round_trip_works() {
        ...
        let metadata = TTDaemonProcessManager::runtime_metadata_for_current_process(&paths)
            .await
            .unwrap();
        TTDaemonProcessManager::write_runtime_metadata(&paths, &metadata)
            .await
            .unwrap();

        let loaded = TTDaemonProcessManager::read_runtime_metadata(&paths)
            .await
            .unwrap();
    ...
    async fn status_marks_stale_metadata_without_socket() {
        ...
        let manager = TTDaemonProcessManager::new(paths.clone(), TTRuntimeOverrides::default());
        let mut metadata = TTDaemonProcessManager::runtime_metadata_for_current_process(&paths)
            .await
            .unwrap();
