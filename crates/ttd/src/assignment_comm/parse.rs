    ...
    match lifecycle {
        ipc::TurnLifecycleState::Interrupted => {
            ...
            parsed.summary = if raw_output.trim().is_empty() {
                "Execution was interrupted before a valid TT report envelope was produced."
                    .to_string()
            } else {
            ...
            parsed.validation.semantic_issues.push(
                "runtime interrupted the turn before TT could trust the report as authoritative"
                    .to_string(),
            );
            warn!(
                assignment_id = %assignment.id,
        ...
        ipc::TurnLifecycleState::Lost | ipc::TurnLifecycleState::Unknown => {
            ...
            parsed.summary = if raw_output.trim().is_empty() {
                "Execution lost runtime continuity before a valid TT report envelope was produced."
                    .to_string()
            } else {
            ...
            parsed.validation.needs_supervisor_review = true;
            parsed.validation.semantic_issues.push(
                "runtime continuity was lost before TT could trust the report as authoritative"
                    .to_string(),
            );
            warn!(
                assignment_id = %assignment.id,
...
pub fn parse_worker_report(
    ...
    let Some(json_payload) = extraction.json_payload else {
        ...
        return recovered_report(
            Some("worker output did not contain exactly one TT report envelope".to_string()),
            assignment,
            record,
        );
    };
...
fn recovered_report(
    ...
    let envelope = WorkerReportEnvelope {
        ...
        task_mode: tt_core::AssignmentTaskMode::Implement,
        ...
        review_signal: tt_core::ReviewSignal {
            level: tt_core::ReviewSignalLevel::Normal,
            reasons: Vec::new(),
            focus: Vec::new(),
        },
    ...
    ParsedWorkerReport {
        envelope: Some(envelope),
        validation: tt_core::WorkerReportValidation {
            validated_at: Utc::now(),
...
mod tests {
    ...
    fn sample_record(assignment: &Assignment) -> tt_core::AssignmentCommunicationRecord {
        let mut collaboration = tt_core::CollaborationState::default();
        collaboration
            .workstreams
    ...
    fn sample_envelope(assignment: &Assignment, packet_id: &str) -> tt_core::WorkerReportEnvelope {
        tt_core::WorkerReportEnvelope {
            ...
            triggered_stop_condition_ids: Vec::new(),
            touched_files: Vec::new(),
            commands_run: vec!["cargo test -p ttd assignment_comm".to_string()],
            artifacts: Vec::new(),
            ...
            mode_payload: WorkerReportModePayload::Implement(ImplementModePayload {
                ...
                tests_run: vec!["cargo test -p ttd assignment_comm".to_string()],
    ...
    fn parse_worker_report_accepts_workspace_report_when_contract_matches() {
        ...
        record.packet.workspace_contract = Some(tt_core::AssignmentWorkspaceContract {
            tracked_thread_id: tt_core::authority::TrackedThreadId::parse("tt-1")
                .expect("tracked thread id"),
            tracked_thread_title: "Workspace thread".to_string(),
            workspace: tt_core::authority::TrackedThreadWorkspace {
                repository_root: "/repo".to_string(),
                owner_tracked_thread_id: tt_core::authority::TrackedThreadId::parse("tt-1")
                    .expect("tracked thread id"),
                strategy:
                    tt_core::authority::TrackedThreadWorkspaceStrategy::DedicatedThreadWorktree,
                ...
                landing_policy:
                    tt_core::authority::TrackedThreadWorkspaceLandingPolicy::MergeToMain,
                sync_policy:
                    tt_core::authority::TrackedThreadWorkspaceSyncPolicy::RebaseBeforeCompletion,
                cleanup_policy:
                    tt_core::authority::TrackedThreadWorkspaceCleanupPolicy::PruneAfterMerge,
                last_reported_head_commit: None,
                status: tt_core::authority::TrackedThreadWorkspaceStatus::Ready,
            },
        });
        let mut envelope = sample_envelope(&assignment, &record.packet.packet_id);
        envelope.workspace_report = Some(tt_core::WorkerWorkspaceReport {
            tracked_thread_id: tt_core::authority::TrackedThreadId::parse("tt-1")
                .expect("tracked thread id"),
            repository_root: "/repo".to_string(),
            worktree_path: "/repo/.worktrees/tt-1".to_string(),
            ...
            workspace_status: tt_core::authority::TrackedThreadWorkspaceStatus::Ahead,
    ...
    fn extract_envelope_rejects_missing_end_marker() {
        let extraction = extract_envelope("TT_REPORT_BEGIN\n{\"summary\":\"unterminated\"}");
    ...
    fn parse_worker_report_marks_surrounding_noise_as_ambiguous_but_keeps_report_contents() {
        ...
        assert!(
            parsed
                ...
                .structural_issues
                .iter()
                .any(|issue| issue.contains("extra text outside the TT report envelope"))
        );
    ...
    fn parse_worker_report_rejects_missing_begin_marker() {
        ...
        assert!(
            parsed
                ...
                .structural_issues
                .iter()
                .any(|issue| issue.contains("did not contain exactly one TT report envelope"))
        );
    }
