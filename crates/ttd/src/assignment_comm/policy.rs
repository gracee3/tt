...
use tt_core::{
    ...
    ReportConfidence, ReportDisposition, ReportParseResult, TTError, TTResult,
...
pub fn validate_assignment_packet(packet: &AssignmentCommunicationPacket) -> TTResult<()> {
    ...
    let fail = |stage: &'static str, error: TTError| -> Result<(), TTError> {
        warn!(
            assignment_id = %packet.assignment_id,
            packet_id = %packet.packet_id,
            stage,
        ...
        || packet.response_contract.marker_end != REPORT_MARKER_END
    {
        return fail(
            ...
            TTError::Protocol(
                "worker report contract markers do not match TT v1 markers".to_string(),
            ),
        );
    }
    debug!(
...
pub fn validate_worker_report_envelope(
    ...
    let mut parse_result = if surrounding_text {
        structural_issues
            .push("worker output contained extra text outside the TT report envelope".to_string());
    ...
    if envelope.review_signal.level == tt_core::ReviewSignalLevel::Required
...
mod tests {
            ...
            communication_seed: Some(AssignmentCommunicationSeed {
                ...
                mode_spec: AssignmentModeSpec::Implement(ImplementModeSpec {
                    expected_verification_commands: vec![
                        "cargo test -p ttd assignment_comm".to_string(),
                    ],
                }),
            }),
    ...
    fn sample_envelope(assignment: &Assignment, packet_id: &str) -> tt_core::WorkerReportEnvelope {
        tt_core::WorkerReportEnvelope {
            ...
            acceptance_results: vec![tt_core::AcceptanceResult {
    ...
    fn validate_assignment_packet_rejects_marker_mismatch() {
        ...
        assert!(
            error
                .to_string()
                .contains("worker report contract markers do not match TT v1 markers")
        );
    }
    ...
    fn validate_worker_report_envelope_accepts_matching_landing_execution_result() {
        let assignment = sample_assignment();
        let mut record = sample_record(&assignment);
        record.packet.landing_execution = Some(tt_core::TrackedThreadLandingExecutionContract {
            tracked_thread_id: tt_core::authority::TrackedThreadId::parse("tt-1")
                .expect("tracked thread id"),
        ...
        let mut envelope = sample_envelope(&assignment, &record.packet.packet_id);
        envelope.landing_execution_result = Some(tt_core::TrackedThreadLandingExecutionResult {
            tracked_thread_id: Some(
                tt_core::authority::TrackedThreadId::parse("tt-1").expect("tracked thread id"),
            ),
            landing_authorization_id: "landing-auth-1".to_string(),
            attempted_head_commit: "head-123".to_string(),
            landing_target: "origin/main".to_string(),
            status: tt_core::TrackedThreadLandingExecutionResultStatus::Succeeded,
