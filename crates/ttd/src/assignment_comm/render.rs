...
use tt_core::{
    ...
    ImplementModePayload, ImplementModeSpec, PromptRenderArtifact, PromptRenderSpec,
    ReportConfidence, ReportDisposition, ReviewSignal, ReviewSignalLevel, TTError, TTResult,
...
pub fn build_assignment_communication_record(
    ...
    now: DateTime<Utc>,
) -> TTResult<AssignmentCommunicationRecord> {
...
pub fn render_prompt(
    ...
    prompt.push_str("You are an TT worker executing one bounded assignment.\n");
    prompt.push_str(
        "TT protocol state is authoritative. Rendered prompt text is derived from TT communication state.\n",
    );
    prompt.push_str(
    ...
    if let Some(workspace_operation) = packet.workspace_operation.as_ref() {
        ...
        prompt.push_str("\n```\n");
        prompt
            .push_str("- This is a standardized TT workspace flow, not freeform prompt wording.\n");
        prompt.push_str("- TT itself remains read-only for git inspection and orchestration.\n");
    ...
    if let Some(prune_workspace) = packet.prune_workspace.as_ref() {
        ...
        prompt.push_str("\n```\n");
        prompt.push_str(
            "- This is a standardized TT workspace cleanup flow, not freeform prompt wording.\n",
        );
        prompt.push_str("- TT itself remains read-only for git inspection and orchestration.\n");
    ...
    if let Some(landing_execution) = packet.landing_execution.as_ref() {
        ...
        prompt.push_str("\n```\n");
        prompt.push_str(
            "- This is a standardized TT landing execution, authorized by a landing authorization record.\n",
        );
        prompt.push_str(
...
fn derive_execution_context(
    ...
    AssignmentExecutionContext {
        runtime_kind: "tt_app_server".to_string(),
        repo_root,
        cwd,
        related_repo_roots,
        requested_model,
...
fn default_acceptance_lines() -> Vec<String> {
    vec![
        "Complete the bounded implement task described in the objective and instructions."
            .to_string(),
        "Return a valid TT worker report envelope with honest implementation details.".to_string(),
    ]
}
...
fn example_workspace_report(
    ...
    let workspace_status = if packet.prune_workspace.is_some() {
        tt_core::authority::TrackedThreadWorkspaceStatus::Pruned
    } else {
...
mod tests {
    ...
    fn sample_seed() -> AssignmentCommunicationSeed {
        AssignmentCommunicationSeed {
            ...
            required_context_refs: vec![
                "crates/ttd/src/assignment_comm/parse.rs".to_string(),
                "crates/ttd/src/assignment_comm/render.rs".to_string(),
            ],
    ...
    fn sample_workspace_contract() -> tt_core::AssignmentWorkspaceContract {
        tt_core::AssignmentWorkspaceContract {
    ...
    fn render_prompt_includes_required_sections_and_example_envelope() {
        ...
        assert!(prompt.contains("You are an TT worker executing one bounded assignment."));
        ...
        assert!(prompt.contains("Response Contract:\n- Emit exactly one JSON envelope between TT_REPORT_BEGIN and TT_REPORT_END."));
    ...
    fn render_prompt_includes_landing_execution_contract_guidance() {
        let collaboration = sample_collaboration();
        let mut seed = sample_seed();
        seed.landing_execution = Some(tt_core::TrackedThreadLandingExecutionContract {
            tracked_thread_id: TrackedThreadId::parse("tt-1").expect("tracked thread id"),
            tracked_thread_title: "Tracked thread".to_string(),
    ...
    fn render_prompt_uses_stable_fallbacks_for_empty_optional_sections() {
        let packet = tt_core::AssignmentCommunicationPacket {
            ...
            execution_context: tt_core::AssignmentExecutionContext {
                runtime_kind: "tt_app_server".to_string(),
            ...
            allowed_scope: tt_core::AssignmentScopeBoundary {
                change_policy: tt_core::AssignmentChangePolicy::CodeAllowed,
            ...
            policy: tt_core::AssignmentCommunicationPolicy {
                stop_at_boundary: true,
                single_report_required: true,
                recommendations_are_non_authoritative: true,
                enforce_scope_boundary: true,
    ...
    fn build_assignment_record_from_seed_includes_source_report_and_context_refs() {
        ...
        assert!(record.packet.included_context.iter().any(|block| {
            block.id == "required_context_refs"
                && block
                    .lines
                    .contains(&"crates/ttd/src/assignment_comm/parse.rs".to_string())
        }));
        assert!(
            record
                .prompt_render
    ...
    fn build_assignment_record_normalizes_relative_cwd_and_renders_execution_context() {
        ...
        assert!(
            record
                .prompt_render
                .prompt_text
                .contains("Runtime kind: tt_app_server")
        );
...
still not a checklist bullet",
        ...
        assert_eq!(
            ...
            vec![
                "Complete the bounded implement task described in the objective and instructions.",
                "Return a valid TT worker report envelope with honest implementation details."
            ]
        );
        assert_eq!(
            record
