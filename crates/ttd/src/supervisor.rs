...
use tt_core::planning::PlanExecutionKind;
...
use tt_core::{
    AppConfig, Assignment, CollaborationState, Decision, DecisionType, Report, ReportDisposition,
    ReportParseResult, SupervisorProposalTrigger, SupervisorProposalTriggerKind,
    SupervisorReasonerUsage, TTError, TTResult, WorkUnit, WorkUnitStatus, WorkerSession,
};
...
impl ResponsesApiReasoner {
    ...
    fn api_key(&self) -> TTResult<Option<String>> {
        ...
        match std::env::var(api_key_env) {
            ...
            Err(std::env::VarError::NotPresent) => Err(TTError::Config(format!(
                "supervisor API key environment variable `{api_key_env}` is not set",
            ))),
            Err(std::env::VarError::NotUnicode(_)) => Err(TTError::Config(format!(
...
) -> TTResult<SupervisorResponseArtifact> {
...
fn repair_supervisor_proposal_value(
    ...
    {
        let drop_revision = match serde_json::from_value::<tt_core::planning::PlanRevisionProposal>(
            plan_revision_value,
        ) {
    ...
    {
        let drop_assessment = match serde_json::from_value::<tt_core::planning::PlanAssessment>(
...
pub fn build_context_pack(
...
) -> TTResult<SupervisorContextPack> {
    let started_at = Instant::now();
    info!(
    ...
    let work_unit = collaboration
        ...
        .get(work_unit_id)
        .cloned()
        .ok_or_else(|| TTError::Protocol(format!("unknown work unit `{work_unit_id}`")))?;
    ...
    let workstream = collaboration
        ...
        .cloned()
        .ok_or_else(|| {
            TTError::Protocol(format!("unknown workstream `{}`", work_unit.workstream_id))
        })?;
...
pub fn validate_proposal(
    ...
    let fail = |stage: &'static str, error: TTError| -> TTResult<()> {
        warn!(
            work_unit_id = %pack.primary_work_unit.id,
            source_report_id = %pack.source_report.id,
            decision_type = snake_label(proposal.proposed_decision.decision_type),
    ...
    if proposal.proposed_decision.target_work_unit_id != pack.primary_work_unit.id {
        return fail(
            "target_work_unit",
            TTError::Protocol("proposal targeted a different work unit".to_string()),
        );
    }
    if proposal.proposed_decision.source_report_id != pack.source_report.id {
        return fail(
            "source_report",
            TTError::Protocol("proposal targeted a different source report".to_string()),
        );
    }
    ...
    if proposal.proposed_decision.requires_assignment != requires_assignment {
        return fail(
            "requires_assignment",
            TTError::Protocol("proposal requires_assignment did not match TT policy".to_string()),
        );
    }
    ...
    match (&proposal.draft_next_assignment, requires_assignment) {
        ...
        (None, true) => {
            return fail(
                ...
                TTError::Protocol("proposal omitted the required draft assignment".to_string()),
            );
        }
        (None, false) => {}
...
fn resolve_source_report(
    ...
    let report_id = source_report_id
        ...
        .ok_or_else(|| {
            TTError::Protocol(format!("work unit `{}` has no latest report", work_unit.id))
        })?;
    ...
    collaboration
        ...
        .get(&report_id)
        .cloned()
        .ok_or_else(|| TTError::Protocol(format!("unknown source report `{report_id}`")))
}
...
fn resolve_current_assignment(
    ...
    collaboration
        ...
        .get(&assignment_id)
        .cloned()
        .ok_or_else(|| TTError::Protocol(format!("unknown assignment `{assignment_id}`")))
}

fn build_workstream_context(
    collaboration: &CollaborationState,
    workstream: &tt_core::Workstream,
...
fn validate_plan_revision_proposal(
    proposal: &tt_core::planning::PlanRevisionProposal,
    pack: &SupervisorContextPack,
) -> TTResult<()> {
    ...
    tt_core::planning::validate_plan_revision_ops(&plan_context.active_plan, &proposal.ops)?;
    Ok(())
}

fn validate_plan_assessment(
    assessment: &tt_core::planning::PlanAssessment,
    pack: &SupervisorContextPack,
) -> TTResult<()> {
    let Some(plan_context) = pack.workstream_plan.as_ref() else {
        return Err(TTError::Protocol(
...
fn runtime_severity(worker_session: &WorkerSession) -> &'static str {
    if matches!(
        ...
        tt_core::WorkerSessionRuntimeStatus::Interrupted
    ) {
        "interrupted"
    } else if matches!(
        worker_session.runtime_status,
        tt_core::WorkerSessionRuntimeStatus::Lost
    ) || matches!(
        worker_session.attachability,
        tt_core::WorkerSessionAttachability::Unknown
    ) {
        "lost_or_unknown"
    } else {
        "clean_terminal"
...
fn label<T>(value: &T) -> TTResult<String>
where
    T: Serialize,
{
    serde_json::to_value(value)?
        .as_str()
        .map(ToOwned::to_owned)
        .ok_or_else(|| TTError::Protocol("failed to serialize protocol label".to_string()))
}
...
mod tests {
    ...
    fn supervisor_prompt_render_is_deterministic_for_same_pack() {
        ...
        assert!(
            first
                .instructions_text
                .contains("You are the TT supervisor reasoner.")
        );
        assert!(
            first
                .instructions_text
    ...
    fn responses_api_reasoner_omits_auth_and_reasoning_for_local_models() {
        let mut config = tt_core::AppConfig::default();
    ...
    fn apply_edits_updates_existing_draft_fields_without_touching_others() {
        ...
        let edits = SupervisorProposalEdits {
            preferred_worker_id: Some("worker-1".to_string()),
            worker_kind: Some("tt-plus".to_string()),
        ...
        assert_eq!(draft.worker_kind.as_deref(), Some("tt-plus"));
