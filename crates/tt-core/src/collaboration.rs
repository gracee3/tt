...
//!
//! This module models the mutable state TT persists for assignments, reports,
...
pub struct CollaborationState {
    ...
    #[serde(default)]
    pub tt_thread_assignments: BTreeMap<String, TTThreadAssignment>,
    #[serde(default)]
...
/// Lifecycle of a TT thread assignment mirror in collaboration state.
///
...
pub enum TTThreadAssignmentStatus {
    Proposed,
    #[default]
    Active,
    Paused,
...
/// Policy that controls who may emit TT prompts for an assignment mirror.
///
/// This is an execution/runtime policy, not a planning authority concept.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum TTThreadSendPolicy {
    #[default]
    HumanApprovalRequired,
    SupervisorMaySend,
}

/// Bootstrap state for a TT thread assignment mirror.
///
/// This tracks the local/runtime bootstrap path only; it does not describe
/// authority planning lifecycle.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum TTThreadBootstrapState {
    NotNeeded,
    #[default]
    Pending,
    Proposed,
...
/// A daemon-side TT assignment mirror tying planning work to a live thread.
///
...
/// A persisted runtime decision attached to a TT thread assignment.
#[derive(Debug, Clone, Serialize, Deserialize)]
