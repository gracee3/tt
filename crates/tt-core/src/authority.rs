...
//!
//! Tracked threads are TT-owned local binding records. They may reference an
...
use crate::{TTError, TTResult, WorkUnitStatus, WorkstreamStatus};
...
impl WorkstreamExecutionScope {
    pub fn validate(&self) -> TTResult<()> {
        require_non_empty(self.tt_home.clone(), "tt_home")?;
...
///
/// This captures how the TT-owned binding relates to an upstream thread
/// identifier. It does not imply that TT owns the upstream thread or the
...
impl TrackedThreadWorkspace {
    pub fn validate_for_owner(&self, owner_tracked_thread_id: &TrackedThreadId) -> TTResult<()> {
...
/// claim that TT owns that thread or its PTY session.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
///
/// This is an TT-owned local binding record that may reference an upstream
...
///
/// Like the record, this is an TT-owned local binding summary rather than an
...
pub trait AuthorityQueryStore: Send + Sync {
    ...
    async fn get_workstream(&self, id: &WorkstreamId) -> TTResult<Option<WorkstreamRecord>>;
    async fn list_work_units(
        &self,
        workstream_id: Option<&WorkstreamId>,
        include_deleted: bool,
    ) -> TTResult<Vec<WorkUnitSummary>>;
    async fn get_work_unit(&self, id: &WorkUnitId) -> TTResult<Option<WorkUnitRecord>>;
...
mod tests {
    ...
    fn tracked_thread_command_round_trips_with_envelope_metadata() {
        let command = AuthorityCommand::CreateTrackedThread(CreateTrackedThread {
            ...
            upstream_thread_id: Some("tt-thread-123".to_string()),
    ...
    fn tracked_thread_record_is_local_and_optional_binding_only() {
        let record = TrackedThreadRecord {
            ...
            notes: None,
            backend_kind: TrackedThreadBackendKind::TT,
            upstream_thread_id: Some("tt-thread-2".to_string()),
        ...
        assert_eq!(summary.upstream_thread_id.as_deref(), Some("tt-thread-2"));
