...
use tt_core::ipc::{
    ...
    AuthorityWorkunitGetRequest, NotificationDeliveryJobListRequest,
    NotificationRecipientUpsertRequest, NotificationSubscriptionListRequest,
    NotificationSubscriptionSetEnabledRequest, NotificationSubscriptionUpsertRequest,
    NotificationTransportKind, OperatorInboxWaitForCheckpointRequest,
    OperatorInboxWaitForCheckpointResponse, OperatorNotificationListRequest,
    ...
    OperatorRemoteActionListRequest, OperatorRemoteActionWaitRequest,
    PlanningSessionApproveRequest, PlanningSessionCreateRequest, PlanningSessionCreateResponse,
    PlanningSessionListRequest, PlanningSessionMarkReadyForReviewRequest,
    PlanningSessionRejectRequest, PlanningSessionRequestResearchRequest,
    PlanningSessionRequestResearchResponse, PlanningSessionRequestSupervisorContextRequest,
    ProposalApproveRequest, ProposalArtifactDetailGetRequest, ProposalArtifactDetailGetResponse,
    ProposalCreateRequest, ProposalCreateResponse, ProposalGetRequest, ProposalGetResponse,
    ProposalRejectRequest, StateGetRequest, TTAssignmentPauseRequest, TTAssignmentResumeRequest,
...
fn client_from_settings(settings: &OperatorServerSettings) -> Result<TTServerClient, String> {
    if settings.server_url.trim().is_empty() {
        return Err("server URL is required".to_string());
    }
    let client = match settings.operator_api_token.as_deref() {
        Some(token) if !token.trim().is_empty() => {
            TTServerClient::with_operator_api_token(settings.server_url.clone(), token)
        }
        _ => TTServerClient::new(settings.server_url.clone()),
    };
    Ok(client)
}
...
fn browser_push_display_name(origin: &str, identity: &storage::BrowserPushIdentity) -> String {
    ...
    format!("TT web browser {origin} ({short})")
}
...
pub async fn create_workstream(
    ...
    title: String,
    objective: String,
    status: tt_core::WorkstreamStatus,
    priority: String,
) -> Result<(), String> {
    let client = client_from_settings(&settings)?;
    client
...
pub async fn delete_workstream(
    ...
    let delete_plan = client
        ...
        .delete_plan;
    client
        .authority_workstream_delete(&tt_core::ipc::AuthorityWorkstreamDeleteRequest {
...
pub async fn pause_tt_assignment(
    settings: OperatorServerSettings,
    assignment_id: String,
) -> Result<(), String> {
    let client = client_from_settings(&settings)?;
    client
        .tt_assignment_pause(&TTAssignmentPauseRequest {
            assignment_id,
...
pub async fn planning_session_create(
    ...
    client
        .planning_session_create(&PlanningSessionCreateRequest {
            ...
            initial_objective,
            research_status: tt_core::PlanningSessionResearchStatus::NotRequested,
...
pub async fn resume_tt_assignment(
    settings: OperatorServerSettings,
    assignment_id: String,
) -> Result<(), String> {
    let client = client_from_settings(&settings)?;
    client
        .tt_assignment_resume(&TTAssignmentResumeRequest {
            assignment_id,
...
pub async fn load_action_request(
    settings: OperatorServerSettings,
    request_id: String,
) -> Result<Option<tt_operator_core::RemoteActionRequestView>, String> {
    let origin = configured_origin(&settings)?;
    let client = client_from_settings(&settings)?;
    let response = client
    ...
    Ok(response
        .request
        .map(tt_operator_core::remote_action_request_view))
}
...
) -> Result<tt_operator_core::RemoteActionRequestView, String> {
    let origin = configured_origin(&settings)?;
    let client = client_from_settings(&settings)?;
    let response = client
        ...
        .await
        .map_err(|error| error.to_string())?;
    Ok(tt_operator_core::remote_action_request_view(
        response.request,
    ))
}
...
pub async fn wait_for_inbox_checkpoint(
    ...
    timeout_ms: Option<u64>,
) -> Result<tt_core::ipc::OperatorInboxWaitForCheckpointResponse, String> {
    let origin = configured_origin(&settings)?;
    let client = client_from_settings(&settings)?;
    client
...
pub async fn inbox_checkpoint(
    settings: OperatorServerSettings,
) -> Result<tt_core::ipc::OperatorInboxMirrorCheckpointQueryResponse, String> {
    let origin = configured_origin(&settings)?;
    let client = client_from_settings(&settings)?;
    client
...
mod tests {
    ...
    fn remote_action_idempotency_key_is_stable_for_the_same_item_state() {
        ...
        let first = storage::remote_action_idempotency_key(
            ...
            tt_core::ipc::OperatorInboxActionKind::Approve,
            updated_at,
        );
        ...
        let different = storage::remote_action_idempotency_key(
            "origin-a",
            "planning_session::session-1",
            tt_core::ipc::OperatorInboxActionKind::Reject,
            updated_at,
        );
    ...
    fn inbox_checkpoint_wait_only_advances_on_new_sequence() {
        let current = tt_core::ipc::OperatorInboxWaitForCheckpointResponse {
            origin_node_id: "origin-a".to_string(),
            checkpoint: tt_core::ipc::OperatorInboxCheckpoint {
                current_sequence: 7,
        ...
        let timed_out = tt_core::ipc::OperatorInboxWaitForCheckpointResponse {
            timed_out: true,
            ..current
        };
