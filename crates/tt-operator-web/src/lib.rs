...
use tt_core::ipc::{OperatorInboxActionKind, OperatorRemoteActionRequestStatus};
use tt_core::{WorkUnitStatus, WorkstreamStatus};
...
fn thread_activity_summary(thread: &tt_core::ipc::ThreadSummary) -> String {
...
fn proposal_status_summary(proposal: &tt_core::ipc::WorkUnitProposalSummary) -> String {
    if proposal.has_generation_failed {
        ...
        tt_core::supervisor::SupervisorProposalStatus::Approved
    ) {
        let decision = proposal
...
fn lane_activity_summary(
    report: Option<&tt_core::ipc::ReportSummary>,
    proposal: Option<&tt_core::ipc::WorkUnitProposalSummary>,
    decision: Option<&tt_core::ipc::DecisionSummary>,
) -> String {
    let mut parts = Vec::new();
...
fn SupervisorProposalDetailBlock(
    proposal: tt_core::supervisor::SupervisorProposalRecord,
    artifact_detail: Option<tt_core::ipc::SupervisorProposalArtifactDetail>,
) -> impl IntoView {
...
fn planning_headline(
    status: &tt_core::PlanningSessionStatus,
    summary: &tt_core::PlanningSessionStructuredSummary,
) -> String {
    match status {
        tt_core::PlanningSessionStatus::Approved => "Plan approved".to_string(),
        tt_core::PlanningSessionStatus::Rejected => "Plan rejected".to_string(),
        tt_core::PlanningSessionStatus::Superseded => "Plan superseded".to_string(),
        tt_core::PlanningSessionStatus::Aborted => "Plan aborted".to_string(),
        tt_core::PlanningSessionStatus::AwaitingApproval => {
            "Waiting for supervisor review".to_string()
        }
        tt_core::PlanningSessionStatus::ResearchRequested => {
            "Supervisor requested research".to_string()
        }
        tt_core::PlanningSessionStatus::Draft | tt_core::PlanningSessionStatus::Chatting => {
...
fn planning_summary_line(
    ...
    match status {
        tt_core::PlanningSessionStatus::Approved => {
            "Supervisor approved this planning session.".to_string()
        }
        tt_core::PlanningSessionStatus::Rejected => {
...
#[component]
fn PlanningWorkflowBlock(session: tt_core::PlanningSession) -> impl IntoView {
...
fn planning_session_is_terminal(status: tt_core::PlanningSessionStatus) -> bool {
    matches!(
        status,
        tt_core::PlanningSessionStatus::Approved
            | tt_core::PlanningSessionStatus::Rejected
            | tt_core::PlanningSessionStatus::Superseded
            | tt_core::PlanningSessionStatus::Aborted
    )
}

#[component]
fn HistoricalPlanningSessionRow(session: tt_core::PlanningSession) -> impl IntoView {
...
#[component]
fn PlanningRecommendationBlock(session: tt_core::PlanningSession) -> impl IntoView {
...
#[component]
fn TurnPlanCard(plan: tt_core::ipc::TurnPlanView) -> impl IntoView {
...
#[component]
fn TokenUsageCard(token_usage: tt_core::ipc::ThreadTokenUsageView) -> impl IntoView {
    view! {
        <div class="json-panel">
...
#[component]
fn TurnItemCard(item: tt_core::ipc::ItemView) -> impl IntoView {
...
#[component]
fn ThreadTurnCard(turn: tt_core::ipc::TurnView) -> impl IntoView {
    ...
    let primary_summary = turn
        ...
        .unwrap_or_else(|| {
            if matches!(turn.status.as_str(), "inProgress" | "in_progress") {
                "TT is still working on this turn.".to_string()
            } else {
...
#[component]
fn ThreadMonitorBlock(detail: tt_core::ipc::ThreadView) -> impl IntoView {
...
fn PlanningThreadMonitorBlock(
    session: tt_core::PlanningSession,
    detail: tt_core::ipc::ThreadView,
) -> impl IntoView {
    let turn_count = detail.turns.len();
    let latest_turn = detail.turns.into_iter().rev().next();
    let activity_summary = thread_activity_summary(&detail.summary);
    ...
    view! {
        <div class="detail-block">
            ...
            {thread.upstream_thread_id.as_ref().map(|thread_id| view! {
                <p class="item-meta">{format!("bound TT thread {thread_id}")}</p>
            })}
        </div>
    }
}
...
fn WorkspaceLifecycleDetailBlock(
    ...
    detail: tt_core::ipc::AuthorityTrackedThreadGetResponse,
) -> impl IntoView {
...
fn PlanningSessionCard(
    ...
    let thread_detail = RwSignal::new(None::<tt_core::ipc::ThreadView>);
    ...
    view! {
        <div class="detail-block">
            ...
            <div class="action-buttons">
                {match status {
                    tt_core::PlanningSessionStatus::Draft | tt_core::PlanningSessionStatus::Chatting | tt_core::PlanningSessionStatus::ResearchRequested => view! {
                        <>
                            <button
                    ...
                    tt_core::PlanningSessionStatus::AwaitingApproval => view! {
                        <>
                            <button
                                class="refresh-button"
                                disabled=move || working.get()
...
fn PlanningSessionsPanel(
    ...
    sessions: Vec<tt_core::PlanningSession>,
    settings: RwSignal<OperatorServerSettings>,
    refresh_epoch: RwSignal<u64>,
    action_message: RwSignal<Option<String>>,
    action_error: RwSignal<Option<String>>,
        ...
        <Router>
            <div class="app-shell">
                <header class="shell-header">
                    <div>
                        <p class="eyebrow">"TT operator web"</p>
...
fn WorkstreamsRoute() -> impl IntoView {
    ...
    view! {
        <PageFrame
            title="Workstreams"
            subtitle="Authority hierarchy grouped by workstream, work unit, and TT thread"
        >
            <div class="toolbar">
            ...
            <article class="card">
                ...
                <div class="action-form">
                    ...
                    <label class="field">
                        ...
                        <textarea
                            rows="3"
                            placeholder="Ship the dashboard slice for workstream and TT thread management."
                    ...
                    if dashboard.hierarchy.workstreams.is_empty() {
                        view! {
                            <EmptyState
                                ...
                                body="Create a workstream to start grouping work units and TT threads."
                            />
                        }
                        .into_any()
                    } else {
...
fn ThreadsRoute() -> impl IntoView {
    ...
    view! {
        <PageFrame
            title="TT Threads"
            subtitle="Live TT app-server threads joined with TT runtime assignment state and authority bindings"
        >
            <div class="toolbar">
                <button class="refresh-button" on:click=move |_| dashboard.refetch()>"Refresh"</button>
                <span class="muted">
                    "Inspect live thread state, attach existing threads to work units, and pause or resume TT-managed assignments."
                </span>
            </div>
            {move || match action_message.get() {
                Some(message) => view! {
...
#[component]
fn LiveThreadCard(
    thread: tt_core::ipc::ThreadSummary,
    ...
    let working = RwSignal::new(false);
    let detail = RwSignal::new(None::<tt_core::ipc::ThreadView>);
    ...
    let attach_title = RwSignal::new(
        thread
            ...
            .clone()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| format!("TT {}", &thread.id[..thread.id.len().min(8)])),
    );
    ...
    let linkage_summary = if let Some(tracked_thread) = linkage.tracked_thread.as_ref() {
        ...
        "No TT lane binding yet".to_string()
    };
        ...
        <article class="card">
            <div class="page-header">
                ...
                <div class="action-buttons">
                    ...
                    {move || match linkage.tt_assignment.clone() {
                        Some(tt_assignment) if matches!(tt_assignment.status, tt_core::TTThreadAssignmentStatus::Active) => view! {
                            <button
                                class="refresh-button"
                                disabled=move || working.get()
                                on:click=move |_| {
                                    let settings = settings.get_untracked();
                                    let assignment_id = tt_assignment.assignment_id.clone();
                                    working.set(true);
                                    action_error.set(None);
                                    #[cfg(target_arch = "wasm32")]
                                    spawn_local(async move {
                                        match api::pause_tt_assignment(settings, assignment_id).await {
                                            Ok(()) => {
                                                action_message.set(Some("Paused TT assignment.".to_string()));
                                                refresh_epoch.update(|value| *value += 1);
                                            }
                                            Err(error) => action_error.set(Some(error)),
                                        }
                        ...
                        Some(tt_assignment) if matches!(tt_assignment.status, tt_core::TTThreadAssignmentStatus::Paused) => view! {
                            <button
                                class="refresh-button"
                                disabled=move || working.get()
                                on:click=move |_| {
                                    ...
                                    #[cfg(target_arch = "wasm32")]
                                    spawn_local(async move {
                                        match api::resume_tt_assignment(settings, assignment_id).await {
                                            Ok(()) => {
                                                action_message.set(Some("Resumed TT assignment.".to_string()));
                                                refresh_epoch.update(|value| *value += 1);
                                            }
                                            Err(error) => action_error.set(Some(error)),
                                        }
                                        ...
                                        spawn_local(async move {
                                            match authority::WorkUnitId::parse(work_unit_id.as_str()) {
                                                Ok(work_unit_id) => match api::create_tracked_thread(
                                                    ...
                                                    Ok(()) => {
                                                        action_message.set(Some("Attached live TT thread to work unit.".to_string()));
                                                        refresh_epoch.update(|value| *value += 1);
                                                        attaching.set(false);
                                                    }
                                                    Err(error) => action_error.set(Some(error)),
...
fn WorkUnitCard(
    ...
    let start_worker_id = RwSignal::new("tt-worker".to_string());
            ...
            <div class="page-header">
                ...
                <div class="action-buttons">
                    ...
                    <button class="refresh-button" on:click=move |_| adding_thread.update(|value| *value = !*value)>
                        {move || if adding_thread.get() { "Close thread form" } else { "Add TT thread" }}
                    </button>
                    <button class="refresh-button" on:click=move |_| starting.update(|value| *value = !*value)>
                        {move || if starting.get() { "Close start form" } else { "Start TT run" }}
                    </button>
                    <button
                        class="refresh-button"
                        disabled=move || working.get()
                                    ...
                                    spawn_local(async move {
                                        ...
                                        .await
                                        {
                                            Ok(()) => {
                                                ...
                                                action_message.set(Some(
                                                    "Added TT thread to work unit.".to_string(),
                                                ));
                                                refresh_epoch.update(|value| *value += 1);
...
fn RuntimeAssignmentCard(
    ...
    assignment: tt_core::ipc::AssignmentSummary,
    dashboard: WorkstreamsDashboardData,
    settings: RwSignal<OperatorServerSettings>,
    refresh_epoch: RwSignal<u64>,
    action_message: RwSignal<Option<String>>,
    ...
    let detail = RwSignal::new(None::<tt_core::ipc::ThreadView>);
    let proposal_record = RwSignal::new(None::<tt_core::supervisor::SupervisorProposalRecord>);
    let proposal_artifact = RwSignal::new(None::<tt_core::ipc::SupervisorProposalArtifactDetail>);
    ...
    let actionable_inbox_item = dashboard
        ...
        .cloned();
    let headline = if assignment.status == tt_core::AssignmentStatus::AwaitingDecision {
    ...
    if let Some(thread) = inferred_thread.as_ref() {
        if assignment.status == tt_core::AssignmentStatus::AwaitingDecision
            && thread.status == "idle"
        {
                                        ...
                                        on:click=move |_| {
                                            ...
                                            spawn_local(async move {
                                                let result = if item_source_kind
                                                    == tt_core::OperatorInboxSourceKind::SupervisorProposal
                                                {
                                                    match action_kind {
                            ...
                            {move || match detail.get() {
                                ...
                                None => {
                                    if loading_detail.get() {
                                        ...
                                        view! { <div class="detail-block"><p class="eyebrow">"Thread monitor"</p><p class="item-meta">"No live TT thread could be inferred for this runtime lane yet."</p></div> }.into_any()
                                    }
                                }
                            }}
                        </div>
...
fn TrackedThreadCard(
    ...
    let loading_proposal = RwSignal::new(false);
    let detail = RwSignal::new(None::<tt_core::ipc::ThreadView>);
    let tracked_detail = RwSignal::new(None::<tt_core::ipc::AuthorityTrackedThreadGetResponse>);
    let proposal_record = RwSignal::new(None::<tt_core::supervisor::SupervisorProposalRecord>);
    let proposal_artifact = RwSignal::new(None::<tt_core::ipc::SupervisorProposalArtifactDetail>);
                    ...
                    on:click=move |_| {
                        ...
                        spawn_local(async move {
                            match api::delete_tracked_thread(settings, tracked_thread_id).await {
                                Ok(()) => {
                                    action_message.set(Some(
                                        "Removed TT thread from work unit.".to_string(),
                                    ));
                                    refresh_epoch.update(|value| *value += 1);
                                }
                                Err(error) => action_error.set(Some(error)),
                                ...
                                None => {
                                    if loading_detail.get() {
                                        ...
                                        view! {
                                            <div class="detail-block">
                                                <p class="eyebrow">"Thread monitor"</p>
                                                <p class="item-meta">"No live TT thread is bound to this tracked thread yet."</p>
                                            </div>
                                        }.into_any()
                                    }
                                }
                                    ...
                                    on:click=move |_| {
                                        ...
                                        spawn_local(async move {
                                            match api::bind_tracked_thread(
                                                ...
                                                Ok(()) => {
                                                    action_message.set(Some("Bound tracked thread to live TT thread.".to_string()));
                                                    refresh_epoch.update(|value| *value += 1);
                                                    binding.set(false);
                                                }
                                                Err(error) => action_error.set(Some(error)),
...
fn render_inbox_section(
    section: tt_operator_core::InboxSectionView,
