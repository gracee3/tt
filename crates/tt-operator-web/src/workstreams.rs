...
pub struct WorkstreamsDashboardData {
    ...
    pub planning_sessions: Vec<tt_core::PlanningSession>,
}
...
pub struct TrackedThreadRuntimeStatusView {
    ...
    pub tt_thread_id: Option<String>,
...
pub struct LiveThreadLinkageView {
    ...
    pub tt_assignment: Option<ipc::TTThreadAssignmentSummary>,
            ...
            dashboard
                ...
                .and_then(|candidate| {
                    dashboard
                        ...
                        .threads
                        .iter()
                        .find(|thread| thread.id == candidate.tt_thread_id)
                        .cloned()
                })
        })
        .or_else(|| {
...
pub fn tracked_thread_runtime_status(
    ...
    let tt_assignment = dashboard
        ...
        .find(|assignment| {
            thread.upstream_thread_id.as_deref() == Some(assignment.tt_thread_id.as_str())
                || assignment.active
        })
        .cloned();
    let assignment = tt_assignment
        .as_ref()
        .and_then(|tt_assignment| {
            dashboard
                .snapshot
                .collaboration
                .assignments
                .iter()
                .find(|assignment| assignment.id == tt_assignment.assignment_id)
                .cloned()
        })
        .or_else(|| {
    ...
    let open_decision = tt_assignment.as_ref().and_then(|tt_assignment| {
        dashboard
            .snapshot
            .collaboration
            ...
            .iter()
            .filter(|decision| {
                decision.assignment_id == tt_assignment.assignment_id && decision.open
            })
    ...
    let headline =
        if assignment.as_ref().is_some_and(|assignment| {
            assignment.status == tt_core::AssignmentStatus::AwaitingDecision
        }) || open_decision.as_ref().is_some_and(|decision| {
            matches!(
                decision.status,
                tt_core::SupervisorTurnDecisionStatus::ProposedToHuman
                    | tt_core::SupervisorTurnDecisionStatus::Approved
            )
        }) {
            "Waiting for supervisor".to_string()
        } else if assignment
            .as_ref()
            .is_some_and(|assignment| assignment.status == tt_core::AssignmentStatus::Running)
            || tt_assignment.as_ref().is_some_and(|assignment| {
                assignment.status == tt_core::TTThreadAssignmentStatus::Active
            })
        {
            "In progress".to_string()
        } else if assignment
            .as_ref()
            .is_some_and(|assignment| assignment.status == tt_core::AssignmentStatus::Created)
        {
            "Queued".to_string()
        } else if tt_assignment.as_ref().is_some_and(|assignment| {
            assignment.status == tt_core::TTThreadAssignmentStatus::Paused
        }) {
            "Paused".to_string()
        } else if assignment.as_ref().is_some_and(|assignment| {
            matches!(
                assignment.status,
                tt_core::AssignmentStatus::Closed
                    | tt_core::AssignmentStatus::Interrupted
                    | tt_core::AssignmentStatus::Failed
                    | tt_core::AssignmentStatus::Lost
            )
        }) {
            humanize_snake_case(
            ...
            matches!(
                assignment.status,
                tt_core::TTThreadAssignmentStatus::Completed
                    | tt_core::TTThreadAssignmentStatus::Released
            )
        }) {
            "Stopped".to_string()
    ...
    if let Some(tt_assignment) = tt_assignment.as_ref() {
        detail_parts.push(format!(
            "tt {}",
            humanize_snake_case(
                serde_json::to_string(&tt_assignment.status)
                    .unwrap_or_default()
                    .trim_matches('"')
            )
        ));
        detail_parts.push(format!(
            "bootstrap {}",
            humanize_snake_case(
                serde_json::to_string(&tt_assignment.bootstrap_state)
                    .unwrap_or_default()
                    .trim_matches('"')
            )
        ));
    ...
    let supervisor_waiting = assignment
        .as_ref()
        .is_some_and(|assignment| assignment.status == tt_core::AssignmentStatus::AwaitingDecision)
    ...
    TrackedThreadRuntimeStatusView {
        ...
        tt_thread_id: tt_assignment.map(|assignment| assignment.tt_thread_id),
        supervisor_waiting,
    }
}
...
pub fn live_thread_linkage(
    ...
    let tt_assignment = dashboard
        ...
        .tt_thread_assignments
        .iter()
        .find(|assignment| assignment.tt_thread_id == thread.id)
        .cloned();
    let assignment = tt_assignment.as_ref().and_then(|tt_assignment| {
        dashboard
            .snapshot
            .collaboration
            .assignments
...
mod tests {
    ...
    use tt_core::events::ConnectionState;
    use tt_core::ipc::{
        AssignmentSummary, DaemonRuntimeMetadata, OperatorInboxState, SessionState, StateSnapshot,
        SupervisorTurnDecisionSummary, TTThreadAssignmentSummary,
    };
    ...
    fn tracked_thread() -> authority::TrackedThreadSummary {
        authority::TrackedThreadSummary {
            ...
            upstream_thread_id: Some("tt-thread-1".to_string()),
    ...
    fn empty_snapshot() -> StateSnapshot {
        StateSnapshot {
            daemon: ipc::DaemonStatusResponse {
                socket_path: "/tmp/ttd.sock".to_string(),
                metadata_path: "/tmp/ttd.json".to_string(),
                tt_endpoint: "ws://127.0.0.1:4500".to_string(),
                tt_binary_path: "/tmp/tt".to_string(),
    ...
    fn reports_waiting_for_supervisor_when_assignment_awaits_decision() {
        ...
        snapshot
            .collaboration
            .tt_thread_assignments
            .push(TTThreadAssignmentSummary {
                assignment_id: "assign-1".to_string(),
                tt_thread_id: "tt-thread-1".to_string(),
                ...
                assigned_at: Utc::now(),
                updated_at: Utc::now(),
                status: tt_core::TTThreadAssignmentStatus::Active,
                send_policy: tt_core::TTThreadSendPolicy::HumanApprovalRequired,
                bootstrap_state: tt_core::TTThreadBootstrapState::Sent,
        ...
        snapshot.collaboration.assignments.push(AssignmentSummary {
            ...
            status: tt_core::AssignmentStatus::AwaitingDecision,
            attempt_number: 1,
            updated_at: Utc::now(),
        });
    ...
    fn reports_in_progress_when_assignment_running() {
        ...
        snapshot.collaboration.assignments.push(AssignmentSummary {
            ...
            tt_thread_id: None,
            tracked_thread_id: None,
            status: tt_core::AssignmentStatus::Running,
            attempt_number: 1,
            updated_at: Utc::now(),
        });
        snapshot
            ...
            .push(TTThreadAssignmentSummary {
                ...
                updated_at: Utc::now(),
                status: tt_core::TTThreadAssignmentStatus::Active,
                send_policy: tt_core::TTThreadSendPolicy::SupervisorMaySend,
                bootstrap_state: tt_core::TTThreadBootstrapState::Sent,
                latest_basis_turn_id: None,
                latest_decision_id: None,
                notes: None,
    ...
    fn reports_waiting_for_supervisor_when_open_decision_exists() {
        ...
        snapshot
            ...
            .push(SupervisorTurnDecisionSummary {
                ...
                basis_turn_id: None,
                kind: tt_core::SupervisorTurnDecisionKind::NextTurn,
                proposal_kind: tt_core::SupervisorTurnProposalKind::ContinueAfterTurn,
                proposed_text: None,
                rationale_summary: "Need review".to_string(),
                status: tt_core::SupervisorTurnDecisionStatus::ProposedToHuman,
                created_at: Utc::now(),
                approved_at: None,
                rejected_at: None,
                sent_at: None,
    ...
    #[test]
    fn inferred_live_thread_prefers_assignment_tt_thread_id() {
        let mut snapshot = empty_snapshot();
        snapshot.threads.push(ipc::ThreadSummary {
            id: "tt-thread-1".to_string(),
        ...
        let assignment = AssignmentSummary {
            ...
            worker_id: "worker-1".to_string(),
            worker_session_id: "session-1".to_string(),
            tt_thread_id: Some("tt-thread-1".to_string()),
            tracked_thread_id: None,
            status: tt_core::AssignmentStatus::AwaitingDecision,
            attempt_number: 1,
            updated_at: Utc::now(),
        ...
        assert_eq!(thread.id, "tt-thread-1");
    }
}
