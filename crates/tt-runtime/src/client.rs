...
use tt_core::ipc::{ThreadTokenUsageView, TurnPlanStepView, TurnPlanView};
use tt_core::{
    ConnectionState, EventEnvelope, ReconnectPolicy, TTError, TTEvent, TTItemEvent, TTResult,
    TTTurnEvent,
};
...
use crate::transport::{ReconnectBackoff, TTTransport, WebSocketTransport};

pub type EventSubscription = broadcast::Receiver<EventEnvelope>;
pub type TTClientHandle = Arc<TTClient>;

type PendingResponse = oneshot::Sender<TTResult<Value>>;

fn item_to_event(item: types::ThreadItem) -> TTItemEvent {
...
fn turn_to_event(turn: types::Turn) -> TTTurnEvent {
...
impl TTClient {
    ...
    pub async fn connect(self: &Arc<Self>) -> TTResult<()> {
        info!(endpoint = %self.transport.endpoint(), "starting TT client connection");
        ...
        match &result {
            ...
            Err(error) => warn!(
                ...
                duration_ms = start.elapsed().as_millis() as u64,
                error = %error,
                "TT client failed to connect before timeout"
            ),
        }
        result
    }
    ...
    pub async fn disconnect(&self) {
        ...
        *self.initialized.lock().await = false;
        ...
        self.emit(EventEnvelope::new(
            ...
            TTEvent::ConnectionStateChanged(ConnectionState {
    ...
    pub async fn initialize(
    ...
    ) -> TTResult<types::InitializeResponse> {
        {
        ...
        let start = Instant::now();
        info!(endpoint = %self.transport.endpoint(), "initializing TT client");
    ...
    ) -> TTResult<types::ModelListResponse> {
    ...
    ) -> TTResult<types::ThreadListResponse> {
    ...
    ) -> TTResult<types::ThreadReadResponse> {
    ...
    ) -> TTResult<types::ThreadStartResponse> {
    ...
    ) -> TTResult<types::ThreadResumeResponse> {
    ...
    ) -> TTResult<types::TurnStartResponse> {
    ...
    ) -> TTResult<types::TurnSteerResponse> {
    ...
    ) -> TTResult<types::TurnInterruptResponse> {
        ...
        T: DeserializeOwned,
    {
        ...
        debug!(
            ...
            method,
            duration_ms = start.elapsed().as_millis() as u64,
            "TT request completed"
        );
        Ok(decoded)
    }

    async fn notify(&self, method: &str, params: Option<Value>) -> TTResult<()> {
        let outbound = self.active_outbound().await?;
        let notification = JsonRpcNotification::new(method, params);
        let raw = serde_json::to_string(&notification)?;
        outbound.send(raw).await.map_err(|error| {
            TTError::Transport(format!("failed to send `{method}` notification: {error}"))
        })
    }

    async fn ensure_initialized(&self) -> TTResult<()> {
        let _guard = self.initialize_gate.lock().await;
        if *self.initialized.lock().await {
            return Ok(());
        }
        let params =
            self.initialize_params.lock().await.clone().ok_or_else(|| {
                TTError::Protocol("TT client has not been initialized".to_string())
            })?;
        let start = Instant::now();
        info!(
            endpoint = %self.transport.endpoint(),
            "initializing TT client after reconnect"
        );
        ...
        *self.initialized.lock().await = true;
        info!(
            ...
            initialized = true,
            duration_ms = start.elapsed().as_millis() as u64,
            "TT client ready after reconnect"
        );
        Ok(())
    }

    async fn active_outbound(&self) -> TTResult<mpsc::Sender<String>> {
        ...
        warn!(
            endpoint = %self.transport.endpoint(),
            connected = false,
            "TT transport is not connected"
        );
        Err(TTError::Transport(format!(
            "TT transport is not connected to {}",
            self.transport.endpoint()
        )))
    }
    ...
    async fn wait_until_connected(&self, timeout: Duration) -> TTResult<()> {
    ...
    async fn connection_loop(self: Arc<Self>) {
        ...
        loop {
            ...
            info!(
                ...
                "starting TT transport connection attempt"
            );
            self.emit(EventEnvelope::new(
                self.transport.endpoint(),
                TTEvent::ConnectionStateChanged(ConnectionState {
            ...
            if !self.reconnect.should_retry(attempt) {
                error!(
                    ...
                    "TT transport exhausted reconnect attempts"
                );
                break;
            }
            ...
            info!(
                ...
                "scheduling TT transport reconnect"
            );
            ...
            let message: JsonRpcMessage = match serde_json::from_str(&raw) {
                ...
                Err(error) => {
                    warn!(
                        endpoint = %self.transport.endpoint(),
                        error = %error,
                        "failed to decode TT JSON-RPC message"
                    );
                    self.emit(EventEnvelope::new(
                        self.transport.endpoint(),
                        TTEvent::Warning {
        ...
        match notification.method.as_str() {
            ...
            methods::REASONING_SUMMARY_TEXT_DELTA => {
                    ...
                    >(params)
                {
                    self.emit(EventEnvelope::new(
                        self.transport.endpoint(),
                        TTEvent::ReasoningSummaryTextDelta {
                            thread_id: event.thread_id,
                            turn_id: event.turn_id,
                            item_id: event.item_id,
                            delta: event.delta,
            ...
            methods::REASONING_SUMMARY_PART_ADDED => {
                    ...
                    >(params)
                {
                    self.emit(EventEnvelope::new(
                        self.transport.endpoint(),
                        TTEvent::ReasoningSummaryPartAdded {
                            thread_id: event.thread_id,
                            turn_id: event.turn_id,
                            item_id: event.item_id,
                            summary_index: event.summary_index,
            ...
            methods::COMMAND_EXECUTION_OUTPUT_DELTA => {
                    ...
                    >(params)
                {
                    self.emit(EventEnvelope::new(
                        self.transport.endpoint(),
                        TTEvent::CommandExecutionOutputDelta {
                            thread_id: event.thread_id,
                            turn_id: event.turn_id,
                            item_id: event.item_id,
                            delta: event.delta,
            ...
            methods::THREAD_TOKEN_USAGE_UPDATED => {
                        ...
                        serde_json::from_value::<types::ThreadTokenUsageUpdatedNotification>(params)
                {
                    self.emit(EventEnvelope::new(
                        self.transport.endpoint(),
                        TTEvent::ThreadTokenUsageUpdated {
                            thread_id: event.thread_id,
...
mod tests {
    ...
    use tt_core::{ReconnectPolicy, TTError};

    use crate::approval::{ApprovalDecision, ApprovalRouter};
    use crate::protocol::jsonrpc::{
        JsonRpcMessage, JsonRpcNotification, JsonRpcRequest, JsonRpcResponse,
    };
    use crate::transport::{TTTransport, TransportConnection};

    use super::*;

    struct ScriptedTransport {
        endpoint: String,
        connections: StdMutex<VecDeque<TTResult<TransportConnection>>>,
    }

    impl ScriptedTransport {
        fn new(connections: Vec<TTResult<TransportConnection>>) -> Self {
            Self {
                endpoint: "test://tt".to_string(),
    ...
    #[async_trait]
    impl TTTransport for ScriptedTransport {
        async fn connect(&self) -> TTResult<TransportConnection> {
            self.connections
                ...
                .pop_front()
                .unwrap_or_else(|| {
                    Err(TTError::Transport(
                        "no scripted TT connections remain".to_string(),
                    ))
                })
        }
    ...
    async fn recv_connection_event(events: &mut EventSubscription) -> String {
        loop {
            ...
            if let TTEvent::ConnectionStateChanged(state) = event.event {
                return state.status;
            }
        }
    }
        ...
        server
            .send_message(JsonRpcMessage::Response(JsonRpcResponse {
                ...
                result: serde_json::to_value(types::InitializeResponse {
                    ...
                    user_agent: Some("tt-tests".to_string()),
    ...
    async fn request_timeout_cleans_up_pending_and_allows_future_requests() {
        ...
        tokio::time::advance(TTClient::REQUEST_TIMEOUT + Duration::from_secs(1)).await;
        let error = timed_out_request
            .await
            .expect("join timed out request")
            .expect_err("request should time out");
        assert!(
            matches!(error, TTError::Transport(message) if message.contains("timed out waiting for `model/list` response"))
        );
    ...
    async fn notifications_and_server_requests_are_mapped_to_events_and_responses() {
        ...
        match events.recv().await.expect("thread started event").event {
            TTEvent::ThreadStarted { thread_id, preview } => {
    ...
    async fn disconnect_fails_pending_request() {
        ...
        assert!(
            matches!(error, TTError::Transport(message) if message.contains("transport disconnected"))
        );
    }
}
