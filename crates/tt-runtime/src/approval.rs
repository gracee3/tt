...
pub trait ApprovalRouter: Send + Sync {
    async fn resolve(&self, method: &str, params: Option<Value>) -> TTResult<ApprovalDecision>;
}
...
impl ApprovalRouter for RejectingApprovalRouter {
    async fn resolve(&self, method: &str, _params: Option<Value>) -> TTResult<ApprovalDecision> {
        Ok(ApprovalDecision::Error {
            code: -32_000,
            message: format!("tt does not yet handle server request `{method}`"),
            data: None,
        })
    }
}
...
impl From<TTError> for ApprovalDecision {
...
mod tests {
    ...
    async fn rejecting_router_returns_method_specific_server_error() {
        ...
        match decision {
            ApprovalDecision::Error {
                ...
                assert_eq!(
                    message,
                    "tt does not yet handle server request `approval/request`"
                );
                assert!(data.is_none());
            }
    ...
    #[test]
    fn tt_error_conversion_preserves_display_message() {
        let decision = ApprovalDecision::from(TTError::Transport("socket unavailable".into()));
