...
#[path = "../../ttd/tests/fake_supervisor.rs"]
mod fake_supervisor;
#[path = "../../ttd/tests/fake_runtime.rs"]
mod fake_tt;
#[path = "../../ttd/tests/harness.rs"]
mod harness;
...
use fake_tt::FakeTTAppServer;
use harness::TestDaemon;
use rusqlite::{Connection, OptionalExtension, params};
use tt_core::{AppConfig, StoredState, authority, ipc};
use tt_core::{
    collaboration::{
...
fn run_tt(daemon: &TestDaemon, args: &[&str]) -> std::process::Output {
    let envs = daemon.tt_home_env();
    run_tt_with_env(args, &envs)
}

fn run_tt_with_env(args: &[&str], envs: &[(String, String)]) -> std::process::Output {
    ...
    command.output().expect("run tt CLI")
}
...
fn save_runtime_state(paths: &tt_core::AppPaths, state: &StoredState) {
...
fn load_runtime_state(paths: &tt_core::AppPaths) -> StoredState {
    let connection = Connection::open(&paths.state_db_file).expect("open runtime sqlite store");
...
struct FakePlanningSessionState {
    ...
    sessions: HashMap<String, tt_core::PlanningSession>,
    next_session_seq: usize,
}

impl FakePlanningSessionDaemon {
    async fn spawn(test_name: &str) -> Self {
        let root = std::env::temp_dir().join(format!("tt-cli-{test_name}-{}", Uuid::new_v4()));
        let paths = tt_core::AppPaths::from_home(root.join(".tt"));
    ...
    fn tt_home_env(&self) -> Vec<(String, String)> {
        vec![(
            "TT_HOME".to_string(),
    ...
    daemon
        ...
        .authority_workstream_create(&ipc::AuthorityWorkstreamCreateRequest {
            command: authority::CreateWorkstream {
                ...
                status: tt_core::WorkstreamStatus::Active,
    ...
    daemon
        ...
        .authority_workunit_create(&ipc::AuthorityWorkunitCreateRequest {
            command: authority::CreateWorkUnit {
                ...
                status: tt_core::WorkUnitStatus::Ready,
            },
        })
        .await
...
async fn spawn_planning_session_cli_daemon(test_name: &str) -> (FakeTTAppServer, TestDaemon) {
        ...
        match method {
            ipc::methods::DAEMON_STATUS => {
                let response = ipc::DaemonStatusResponse {
                    ...
                    tt_endpoint: "ws://fake-tt".to_string(),
                    tt_binary_path: "/usr/bin/tt".to_string(),
                    upstream: tt_core::ConnectionState {
                        endpoint: "ws://fake-tt".to_string(),
                        status: "connected".to_string(),
                        detail: None,
                    },
                    client_count: 1,
                    known_threads: 1,
                    runtime: tt_core::DaemonRuntimeMetadata {
                        pid: std::process::id(),
                        started_at: Utc::now(),
            ...
            ipc::methods::PLANNING_SESSION_CREATE => {
                ...
                let now = Utc::now();
                let session = tt_core::PlanningSession {
                    session_id: session_id.clone(),
                    workstream_id: params.workstream_id.clone(),
                    status: tt_core::PlanningSessionStatus::Chatting,
                    planning_thread_id: params
                        .planning_thread_id
                        .unwrap_or_else(|| format!("planning-thread-{session_id}")),
                    base_plan_id: Some(
                        tt_core::planning::PlanId::parse(format!("plan-{}", params.workstream_id))
                            .expect("plan id"),
                    ),
                    ...
                    latest_structured_summary: tt_core::PlanningSessionStructuredSummary {
...
fn seed_cli_planning_session(
    ...
    let now = Utc::now();
    let workstream = tt_core::collaboration::Workstream {
    ...
    let active_plan = if include_active_plan {
        let active_plan = tt_core::planning::WorkstreamPlan::bootstrap_from_workstream(
            &workstream,
            &[],
            "cli_operator",
            now,
    ...
    stored.collaboration.planning_sessions.insert(
        ...
        tt_core::collaboration::PlanningSession {
            session_id: session_id.to_string(),
            workstream_id: workstream_id.to_string(),
            status: if ready_for_review {
                tt_core::collaboration::PlanningSessionStatus::AwaitingApproval
            } else {
                tt_core::collaboration::PlanningSessionStatus::Chatting
            },
            ...
            approved_plan_id: None,
            approved_plan_version: None,
            latest_structured_summary: tt_core::collaboration::PlanningSessionStructuredSummary {
                objective: objective.to_string(),
                research_status:
                    tt_core::collaboration::PlanningSessionResearchStatus::NotRequested,
...
async fn spawn_assignment_ready_daemon(
    test_name: &str,
) -> (FakeTTAppServer, TestDaemon, ipc::AssignmentStartResponse) {
    let fake_tt = FakeTTAppServer::spawn().await;
    let daemon = TestDaemon::spawn_with_env(
        test_name,
        vec![(
...
async fn spawn_proposal_ready_daemon(
    ...
    let mut daemon = TestDaemon::spawn_with_env(
        ...
        vec![
            ...
            (
                "TT_TEST_SUPERVISOR_API_KEY".to_string(),
    ...
    (fake_tt, fake_supervisor, daemon, started)
}

async fn spawn_tt_review_ready_daemon(
    test_name: &str,
) -> (FakeTTAppServer, TestDaemon, ipc::TTAssignmentCreateResponse) {
    let fake_tt = FakeTTAppServer::spawn().await;
    let daemon = TestDaemon::spawn_with_env(
        test_name,
        vec![(
    ...
    let assignment = client
        .tt_assignment_create(&ipc::TTAssignmentCreateRequest {
            tt_thread_id: thread.id,
        ...
        .await
        .expect("create tt assignment");

    (fake_tt, daemon, assignment)
}
...
async fn real_cli_can_connect_to_daemon_and_read_basic_state() {
    ...
    let output = run_tt(&daemon, &["daemon", "status"]);
...
async fn real_cli_doctor_reports_current_runtime_and_persistence_paths() {
    ...
    let output = run_tt(&daemon, &["doctor"]);
    assert!(output.status.success(), "stderr: {}", stderr(&output));
    ...
    assert!(field_value(&stdout, "tt_endpoint").is_some());
...
async fn real_cli_can_read_workunit_detail_after_real_setup() {
    ...
    assert!(stdout.contains(&format!(
        "tracked_thread\t{}\trev=1\tTT\tBound\tCLI Detail Thread",
        tracked_thread.id
    )));
...
async fn real_cli_can_create_workstream_and_read_it_back() {
    ...
    let get_output = run_tt(&daemon, &["project", "get", "--workstream", &workstream_id]);
    assert!(
    ...
    assert!(get_stdout.contains("origin_node_id: tt-cli"));
...
async fn real_cli_can_create_workunit_and_read_it_back() {
    ...
    let get_workunit = run_tt(&daemon, &["worktree", "get", "--workunit", &work_unit_id]);
    assert!(
    ...
    assert!(get_workunit_stdout.contains("origin_node_id: tt-cli"));
...
async fn real_cli_can_edit_and_delete_authority_workstream() {
    ...
    assert_eq!(stored.workstream.status, tt_core::WorkstreamStatus::Blocked);

    let delete_output = run_tt(&daemon, &["project", "delete", &workstream_id]);
    assert!(
...
async fn real_cli_can_create_edit_and_delete_tracked_thread_via_canonical_cli() {
    ...
    let create_tracked_thread = run_tt(
        &daemon,
        &[
            "worktree",
            "thread",
            ...
            "/tmp/tt-cli",
            "--notes",
            "Track this TT binding through the canonical CLI surface.",
            "--upstream-thread",
            "thread-upstream-1",
            "--model",
            "gpt-5.4",
    ...
    assert!(get_stdout.contains("backend_kind: TT"));
    assert!(get_stdout.contains("binding_state: Bound"));
    assert!(get_stdout.contains("preferred_cwd: /tmp/tt-cli"));
...
async fn real_cli_can_read_report_state_after_real_assignment_setup() {
    let (_fake_tt, mut daemon, started) = spawn_assignment_ready_daemon("cli-report-get").await;
...
async fn real_cli_can_read_assignment_state_after_real_assignment_setup() {
    let (_fake_tt, mut daemon, started) = spawn_assignment_ready_daemon("cli-assignment-get").await;
...
async fn real_cli_planning_session_approve_stages_revision_proposal_without_applying_plan() {
    ...
    let (_fake_tt, mut daemon) = spawn_planning_session_cli_daemon("cli-planning-approve").await;
...
async fn real_cli_planning_session_create_rejects_ready_for_review_shortcut_and_keeps_draft_create_working()
 {
    ...
    let invalid_create = run_tt_with_env(
        &[
            "supervisor",
            "plan",
            "create",
    ...
    let empty_list = run_tt_with_env(
    ...
    let valid_create = run_tt_with_env(
        &[
            "supervisor",
            "plan",
            "create",
    ...
    let created_list = run_tt_with_env(
        &["supervisor", "plan", "list", "--workstream", workstream_id],
        &envs,
    );
    assert!(
...
async fn real_cli_planning_session_request_research_succeeds_once_and_rejects_repeat() {
    ...
    let first_output = run_tt_with_env(
        &[
            "supervisor",
            "plan",
            "request-research",
    ...
    let second_output = run_tt_with_env(
        &[
            "supervisor",
            "plan",
            "request-research",
...
async fn real_cli_planning_session_update_summary_stays_descriptive_and_rejects_ready_smuggling() {
    let _guard = planning_session_cli_test_lock().lock().await;
    let (_fake_tt, mut daemon) = spawn_planning_session_cli_daemon("cli-planning-update").await;
...
async fn real_cli_planning_session_mark_ready_for_review_explicitly_transitions_and_rejects_invalid_state()
 {
    let _guard = planning_session_cli_test_lock().lock().await;
    let (_fake_tt, mut daemon) = spawn_planning_session_cli_daemon("cli-planning-ready").await;
...
async fn real_cli_planning_session_help_mentions_lifecycle_boundaries() {
    ...
    let root_help = run_tt(&daemon, &["supervisor", "plan", "--help"]);
    ...
    let create_help = run_tt(&daemon, &["supervisor", "plan", "create", "--help"]);
    assert!(
    ...
    let approve_help = run_tt(&daemon, &["supervisor", "plan", "approve", "--help"]);
    assert!(
    ...
    let update_help = run_tt(&daemon, &["supervisor", "plan", "update-summary", "--help"]);
    assert!(
...
async fn real_cli_can_list_reports_for_workunit_after_real_assignment_setup() {
    let (_fake_tt, mut daemon, started) = spawn_assignment_ready_daemon("cli-report-list").await;
...
async fn real_cli_can_apply_decision_after_real_assignment_setup() {
    let (_fake_tt, mut daemon, started) = spawn_assignment_ready_daemon("cli-decision-apply").await;

    let apply_output = run_tt(
        &daemon,
        &[
            "supervisor",
            "work",
...
async fn real_cli_can_discover_pending_review_item_via_queue_and_history() {
    let (_fake_tt, mut daemon, assignment) = spawn_tt_review_ready_daemon("cli-review-queue").await;
    let pending_decision_id =
        pending_review_decision_id(&daemon, &assignment.assignment.assignment_id).await;

    let queue_output = run_tt(
        &daemon,
        &[
            "supervisor",
            "review",
...
async fn real_cli_can_fetch_pending_review_item_detail_via_get() {
    let (_fake_tt, mut daemon, assignment) = spawn_tt_review_ready_daemon("cli-review-get").await;
    let pending_decision_id =
        pending_review_decision_id(&daemon, &assignment.assignment.assignment_id).await;
...
async fn real_cli_can_approve_pending_review_item() {
    let (_fake_tt, mut daemon, assignment) =
        spawn_tt_review_ready_daemon("cli-review-approve").await;
    let pending_decision_id =
        pending_review_decision_id(&daemon, &assignment.assignment.assignment_id).await;
...
async fn real_cli_can_reject_pending_review_item() {
    let (_fake_tt, mut daemon, assignment) =
        spawn_tt_review_ready_daemon("cli-review-reject").await;
    let pending_decision_id =
        pending_review_decision_id(&daemon, &assignment.assignment.assignment_id).await;

    let reject_output = run_tt(
        &daemon,
        &[
            "supervisor",
            "review",
...
async fn real_cli_review_queue_and_history_reflect_approval_transition() {
    let (_fake_tt, mut daemon, assignment) =
        spawn_tt_review_ready_daemon("cli-review-transition").await;
    let pending_decision_id =
        pending_review_decision_id(&daemon, &assignment.assignment.assignment_id).await;

    let queue_before = run_tt(
        &daemon,
        &[
            "supervisor",
            "review",
...
async fn real_cli_reports_missing_review_item_with_nonzero_exit() {
    let (_fake_tt, mut daemon, _assignment) =
        spawn_tt_review_ready_daemon("cli-review-missing").await;
