...
impl LiveSession {
    fn launch(
        ...
        let pair = pty_system
            ...
            .context("open pty for TT TUI")?;

        let mut command = CommandBuilder::new(service.config.tt.binary_path.clone());
        command.arg("--remote");
        command.arg(service.config.tt.effective_listen_url());
        command.arg("resume");
        command.arg(&thread.id);
        command.env("RUNTIME_HOME", service.shared_app_server_tt_home());
        command.env(
            "RUNTIME_SQLITE_HOME",
    ...
    fn send_key(&self, key: KeyEvent) -> Result<()> {
        ...
        writer
            .write_all(&bytes)
            .context("write key event to TT PTY")?;
        writer.flush().ok();
        Ok(())
    }
    ...
    fn terminate(&self) -> Result<()> {
        if let Ok(mut child) = self.child.lock() {
            child.kill().context("terminate TT child")?;
...
pub async fn run_dashboard(service: SupervisorService) -> Result<()> {
    if !io::stdout().is_terminal() {
        bail!("tt tui requires an interactive terminal");
    }
