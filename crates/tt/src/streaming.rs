...
use tt_core::{AppConfig, AppPaths, TurnStartRequest, ipc};
use ttd::{
    EventSubscription, TTDaemonLaunch, TTDaemonProcessManager, TTIpcClient, TTRuntimeOverrides,
};
...
#[derive(Debug, Clone)]
pub struct TTSupervisorStreamingBackend {
    paths: AppPaths,
    daemon: TTDaemonProcessManager,
    initial_launch: TTDaemonLaunch,
}

impl TTSupervisorStreamingBackend {
    pub fn new(paths: AppPaths, config: &AppConfig, overrides: &TTRuntimeOverrides) -> Self {
        let initial_launch = if overrides.force_spawn {
            TTDaemonLaunch::Always
        } else {
            match config.tt.connection_mode {
                tt_core::TTConnectionMode::ConnectOnly => TTDaemonLaunch::Never,
                tt_core::TTConnectionMode::SpawnIfNeeded => TTDaemonLaunch::IfNeeded,
                tt_core::TTConnectionMode::SpawnAlways => TTDaemonLaunch::Always,
            }
        };
        Self {
            daemon: TTDaemonProcessManager::new(paths.clone(), overrides.clone()),
            paths,
            initial_launch,
        }
    }
...
#[async_trait]
impl SupervisorStreamingBackend for TTSupervisorStreamingBackend {
    type Client = Arc<TTIpcClient>;
    ...
    async fn reconnect_reports_interruption_honestly_when_turn_cannot_be_reanchored() {
        let backend = FakeBackend::default();
        {
            ...
            state.attachments = VecDeque::from(vec![Ok(ipc::TurnAttachResponse {
                ...
                attached: false,
                reason: Some(
                    "turn continuity was lost when TT lost daemon/upstream ownership".to_string(),
                ),
            })]);
            state.start_turn_response = Some(ipc::TurnStartResponse {
                turn_id: "turn-1".to_string(),
