...
Role identity belongs in lane-specific `developer_instructions` or `.tt/agents/*.toml` files.
...
Put these in lane-specific `developer_instructions` or `.tt/agents/*.toml`:
